...
1. Find the key `AUTHORITY` and replace the existing value with https://login.microsoftonline.com/ReplaceWithTenantID.
1. Find the key `SCOPE` and replace the existing value with https://flaskapi/.default.
...
1. In the app's registration screen, select **Authentication** in the menu.
   - In the **Redirect URIs** section, update the reply URLs to match the site URL of your Azure deployment. For example:
      - `https://PythonDjangoWebApp.azurewebsites.net/account/callback`

### Update authentication configuration parameters (Python Django Web App)

1. In your IDE, locate the `DjangoUI` project. Then, open `DjangoUI\production.env`.
2. Find the key for **REDIRECT_URI** and replace its value with the address of the web app you published, for example, [https://PythonDjangoWebApp.azurewebsites.net/redirect](https://PythonDjangoWebApp.azurewebsites.net/redirect).
3. Find the key for **FLASK_BACKEND_URL** and replace its value with the address of the web API you published, for example, [https://PythonFlaskWebAPI.azurewebsites.net/api](https://PythonFlaskWebAPI.azurewebsites.net/api).
