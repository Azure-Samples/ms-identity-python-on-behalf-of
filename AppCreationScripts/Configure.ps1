...
#> 
   ...
   $clientAadApplication = New-AzureADApplication -DisplayName "Python Django Web App" `
                                                  ...
                                                  -IdentifierUris "https://$tenantName/PythonDjangoWebApp" `
                                                  -PasswordCredentials $key `
                                                  -PublicClient $False
   ...
   Write-Host "Updating the sample code ($configFile)"
   $dictionary = @{ "APIAppRegistrationClientID" = $serviceAadApplication.AppId;"APIAppRegistrationClientSecret" = $serviceAppKey;"ReplaceWithTenantID" = $tenantId };
   ReplaceInTextFile -configFilePath $configFile -dictionary $dictionary
   Write-Host ""
