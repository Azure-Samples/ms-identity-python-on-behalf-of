...
#> 
   ...
   Write-Host "Creating the AAD application (Python Flask Web API)"
   ...
   $serviceAadApplication = New-AzureADApplication -DisplayName "Python Flask Web API" `
   ...
   Write-Host "Done creating the service application (Python Flask Web API)"
   ...
   Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>Python Flask Web API</a></td></tr>" -Path createdApps.html

   $requiredResourcesAccess = New-Object System.Collections.Generic.List[Microsoft.Open.AzureAD.Model.RequiredResourceAccess]

   # Add Required Resources Access (from 'service' to 'Windows Azure Service Management API')
   Write-Host "Getting access from 'service' to 'Windows Azure Service Management API'"
   ...
   Write-Host "Creating the AAD application (Python Django Web App)"
   ...
   $clientAppKey = $pw
   # create the application 
   $clientAadApplication = New-AzureADApplication -DisplayName "Python Django Web App" `
                                                  -HomePage "http://localhost:8000/" `
                                                  -ReplyUrls "http://localhost:8000/account/callback" `
                                                  -IdentifierUris "https://$tenantName/Python Django Web App" `
                                                  -PasswordCredentials $key `
                                                  -PublicClient $False
   ...
   Write-Host "Done creating the client application (Python Django Web App)"

   # URL of the AAD application in the Azure portal
   # Future? $clientPortalUrl = "https://portal.azure.com/#@"+$tenantName+"/blade/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/Overview/appId/"+$clientAadApplication.AppId+"/objectId/"+$clientAadApplication.ObjectId+"/isMSAApp/"
   $clientPortalUrl = "https://portal.azure.com/#blade/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/CallAnAPI/appId/"+$clientAadApplication.AppId+"/objectId/"+$clientAadApplication.ObjectId+"/isMSAApp/"
   Add-Content -Value "<tr><td>client</td><td>$currentAppId</td><td><a href='$clientPortalUrl'>Python Django Web App</a></td></tr>" -Path createdApps.html
   ...
   # Add Required Resources Access (from 'client' to 'MIcrosoft Graph API')
   Write-Host "Getting access from 'client' to 'MIcrosoft Graph API'"
   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "Microsoft Graph API" `
                                                -requiredDelegatedPermissions "openid" `
   ...
   $dictionary = @{ "CLIENT_ID" = $clientAadApplication.AppId;"CLIENT_SECRET" = $clientAppKey;"DJANGO_SECRET_KEY" = Secret Key Generated By Python Django Framework;"AUTHORITY" = https://login.microsoftonline.com/$tenantId;"SCOPE" = https://flaskapi/.default;"API_SCOPE" = api://flaskapi/Subscriptions.Read };
   ReplaceInTextFile -configFilePath $configFile -dictionary $dictionary

   # Update config file for 'service'
   $configFile = $pwd.Path + "\..\FlaskAPI\production.env"
   Write-Host "Updating the sample code ($configFile)"
   $dictionary = @{ "CLIENT_ID" = $serviceAadApplication.AppId;"CLIENT_SECRET" = $serviceAppKey;"AUTHORITY" = https://login.microsoftonline.com/$tenantId;"ISSUER" = https://login.microsoftonline.com/$tenantId/v2.0 };
   ReplaceInTextFile -configFilePath $configFile -dictionary $dictionary
   Write-Host ""
